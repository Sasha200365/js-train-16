...
function filterStudentsByGrade(students, grade) {
  ...
  // const result = students.filter((grade) => grade == "A");
  const studA = (value, index, students) => grade === "A";
  ...
  const filterArr = students.filter(studA);
...
function compareArrays(arr1, arr2) {
  ...
  const jsonArr1 = JSON.stringify(arr1, (key, value) => {});
  const jsonArr2 = JSON.stringify(arr2, (key, value) => {});
  const bulev = jsonArr1 != jsonArr2;
  console.assert(bulev, "Mасиви не однакові");
}
 ...
 */
function displayGroupedInfo(student) {
  // Переберомо об'єкт за допомого for of

  for (let values of student) {
...
function validateUserInput(userInput) {
  ...
  // const passLeng = password.length < 8;
  // if (passLeng) {
